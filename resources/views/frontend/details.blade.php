					...
					<div class="row justify-content-between">
						...
						<div class="col-xl-5 col-lg-6 col-md-12 col-sm-12">
							<div class="quick_view_slide">
								...
								<div class="single_view_slide"><a href="{{ asset('uploads/product/thumbnail') }}/{{ $thumbnail->thumbnail }}" data-lightbox="roadtrip" class="d-block mb-4"><img src="{{ asset('uploads/product/thumbnail') }}/{{ $thumbnail->thumbnail }}" class="img-fluid rounded" alt="" /></a></div>
						...
						<div class="col-xl-7 col-lg-6 col-md-12 col-sm-12">
							<div class="prd_details pl-3">
								...
								<div class="prt_01 mb-1"><span class="text-light bg-info rounded px-2 py-1">{{ $categories->first()->category_name }}</span></div>
								<div class="prt_02 mb-3">
									<h2 class="ft-bold mb-1">{{ $products->first()->product_name }}</h2>
									<div class="text-left">
										...
										<div class="elis_rty">
											@if ($products->first()->discount != null)
												...
												<span class="ft-medium text-muted line-through fs-md mr-2">৳{{ $products->first()->price }}</span>
												@endif
											{{-- <span class="ft-medium text-muted line-through fs-md mr-2">৳{{ $products->first()->price }}</span> --}}
											<span class="ft-bold theme-cl fs-lg mr-2">৳{{ $products->first()->after_discount }}</span>
										</div>
									</div>
								</div>
								
								<div class="prt_03 mb-4">
									<p>{{ $products->first()->short_desp }}</p>
								</div>
								...
								<div class="prt_04 mb-2">
									...
									<div class="text-left">
										...
										@foreach ($inventories as $inventory )
											@if ($inventory->rel_to_color->color_code == null)
												<h5 class="text-danger">Not Availabe</h5>
												<input type="hidden" id="color_id" value="1" name="color_id">
												@else
												<div class="form-check form-option form-check-inline mb-1">
													<input class="form-check-input color_id" type="radio" name="color_id" id="white{{ $inventory->rel_to_color->id }}" value="{{ $inventory->rel_to_color->id }}">
													<label class="form-option-label rounded-circle" for="white{{ $inventory->rel_to_color->id }}">
													<span class="form-option-color rounded-circle" style="background: #{{ $inventory->rel_to_color->color_code }}"></span></label>
												</div>
											@endif
												
											@php
												$colors = $inventory->rel_to_color->color_code;
											@endphp
									...
									<div class="text-left pb-0 pt-2" id="size_id">
										@if ($colors != null)
											...
											<div class="form-check size-option form-option form-check-inline mb-2">
												<input class="form-check-input" type="radio" name="size_id" id="{{ $size->id }}" >
										...
										@else
											...
											<div class="form-check size-option form-option form-check-inline mb-2">
												<input class="form-check-input" checked type="radio" name="size_id" id="{{ $size->rel_to_size->id }}" value="{{ $size->rel_to_size->id }}" >
												<label class="form-option-label" for="{{ $size->rel_to_size->id }}">{{ $size->rel_to_size->size_name }}</label>
											</div>
											@else
											<div class="form-check size-option form-option form-check-inline mb-2">
												<input class="form-check-input id_size" type="radio" name="size_id" id="{{ $size->rel_to_size->id }}" value="{{ $size->rel_to_size->id }}" >
												<label class="form-option-label" for="{{ $size->rel_to_size->id }}">{{ $size->rel_to_size->size_name }}</label>
											</div>
											@endif
								...
								<div class="prt_05 mb-4">
									<div class="form-row mb-7">
										...
										<form action="{{ route('wishlist') }}" method="POST">
											...
											<div class="col-12 col-lg-auto">
												<!-- Wishlist --> 
												<input type="hidden" value="{{ $products->first()->id }}" name="product_id">
								  ...
								  @if (session('stock_out'))
												<div class="alert alert-danger text-center">{{ session('stock_out') }}</div>
											@endif
								</div>
									...
									<div class="product_grid card b-0 mb-0">
										...
										<div class="card-body p-0">
											<div class="shop_thumb position-relative">
												<a class="card-img-top d-block overflow-hidden" href="{{ route('details', $rel_product->slug) }}"><img class="card-img-top" src="{{ asset('uploads/product/preview') }}/{{ $rel_product->preview }}" alt="..."></a>
											</div>
										</div>
										<div class="card-footer b-0 p-3 pb-0 d-flex align-items-start justify-content-center">
											<div class="text-left">
												<div class="text-center">
													<h5 class="fw-bolder fs-md mb-0 lh-1 mb-1"><a href="shop-single-v1.html">{{ $rel_product->product_name }}</a></h5>
...
@endif
{{-- WISHLIST ADDED SUCCESS MESSAGE --}}
@if (session('wl_success'))
<script>
	const Toast = Swal.mixin({
  toast: true,
  position: 'top-end',
...
Toast.fire({
  icon: 'success',
  title: '{{ session('wl_success') }}'
})
</script>

@endif
{{-- WISHLIST EXIST MESSAGE --}}
@if (session('wishlist_extists'))
<script>
	Swal.fire({
  title: '{{ session('wishlist_extists') }}',
  showClass: {
